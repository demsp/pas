LABEL prev,next;
var
  str_arr:array[1..10] of string; //сигнатура функции
...
com_mem:='!b:$+#.!c:3*$.!d:$-#.~b 5 3.~c 3.%d: c^ b^.b#c#d#a#';
...
prev:
 ...
 if(com_mem[i]='^')then  data_arr[1]:=data_arr[str_index];
...
//запихиваем строку обратно в str_arr
 ...
 data_arr[1]:=integer(str[j])-integer('0');
 //
 first_flag:=true;
 j_first_stor:=j;
 end;
 if (first_flag=true)and(j<>j_first_stor) then begin
   ...
   //if(sight='-') then acc:=acc-data_arr[j];
   //if(sight='*') then acc:=acc*data_arr[j];
   if(sight='+') then data_arr[1]:=data_arr[1]+data_arr[j];
   if(sight='-') then data_arr[1]:=data_arr[1]-data_arr[j];
   if(sight='*') then data_arr[1]:=data_arr[1]*data_arr[j];
   //
   end;
   ...
   //if(sight='-') then acc:=acc-(integer(str[j])-integer('0'));
   //if(sight='*') then acc:=acc*(integer(str[j])-integer('0'));
   if(sight='+') then data_arr[1]:=data_arr[1]+(integer(str[j])-integer('0'));
   if(sight='-') then data_arr[1]:=data_arr[1]-(integer(str[j])-integer('0'));
   if(sight='*') then data_arr[1]:=data_arr[1]*(integer(str[j])-integer('0'));
   //
   str[j]:='~';
   end;
 data_arr[str_index_stor]:=data_arr[1];
